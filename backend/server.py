...
def classify(req: JDRequest):
    ...
    # --- Extract company & recruiter heuristically ---
    company_match = re.search(r"(?:at|with)\s+([A-Z][A-Za-z0-9& ]+)", req.text)
    recruiter_match = re.search(r"(?:contact|reach out to|report to)\s+([A-Z][A-Za-z ]+)", req.text)

    company = company_match.group(1).strip() if company_match else "Unknown Company"
    recruiter = recruiter_match.group(1).strip() if recruiter_match else "Hiring Manager"

  # --- Extract recruiter email ---
    email_match = re.search(r'[\w\.-]+@[\w\.-]+\.\w+', req.text)
    recruiter_email = email_match.group(0) if email_match else ""

    return {
        "jobType": result["labels"][0],
        "jobTitle": job_title,
...
# Fallback logic:
    sender_name = req.senderName if req.senderName else "Your Name"
    company = req.company if req.company else "the company"
    recruiter = req.recruiterName if req.recruiterName else "Hiring Manager"
    job_title = req.jobTitle if req.jobTitle else "the role"
